...
/// TV-optimized keyboard dialog for entering text with a D-pad remote.
...
class _TvKeyboardDialogState extends State<_TvKeyboardDialog> {
  ...
  // Last row has special action keys
  static const _actionRow = ['BKSP', 'CLEAR', 'DONE'];

  String get _displayText => _buffer.toString();

  void _onKey(LogicalKeyboardKey key) {
    setState(() {
      if (key == LogicalKeyboardKey.arrowRight) {
        _cursorCol = (_cursorCol + 1).clamp(0, 9);
      } else if (key == LogicalKeyboardKey.arrowLeft) {
        _cursorCol = (_cursorCol - 1).clamp(0, 9);
      } else if (key == LogicalKeyboardKey.arrowDown) {
        if (_cursorRow < _rows.length - 1) {
          _cursorRow++;
          _cursorCol = _cursorCol.clamp(0, _rows[_cursorRow].length - 1);
        } else {
          // Move to action row
          _cursorRow = _rows.length;
          _cursorCol = (_cursorCol ~/ 3).clamp(0, _actionRow.length - 1);
        }
        ...
        if (_cursorRow < _rows.length) {
          final char = _rows[_cursorRow][_cursorCol];
          if (char.isNotEmpty) _buffer.write(char);
        } else {
          final action = _actionRow[_cursorCol.clamp(0, _actionRow.length - 1)];
          if (action == 'BKSP') {
            ...
            widget.onSubmitted(_buffer.toString());
            return;
          }
        }
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    color: _displayText.isEmpty ? Colors.white38 : Colors.white,
                    fontSize: 18,
              ...
              const SizedBox(height: 16),
              // Character grid
              ...List.generate(_rows.length, (row) {
                return Row(
                  children: List.generate(_rows[row].length, (col) {
                    final char = _rows[row][col];
                    final isActive = row == _cursorRow && col == _cursorCol;
                    if (char.isEmpty) return const SizedBox(width: 36, height: 36);
                    return GestureDetector(
                      onTap: () {
                        setState(() {
                          _cursorRow = row;
              ...
              Row(
                ...
                children: List.generate(_actionRow.length, (i) {
                  final action = _actionRow[i];
                  final isActive = _cursorRow == _rows.length && i == _cursorCol;
                  return GestureDetector(
                    onTap: () {
                      setState(() {
                        _cursorRow = _rows.length;
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isActive
                            ? (action == 'DONE' ? Colors.green.withValues(alpha: 0.3) : AppTheme.current.primaryColor.withValues(alpha: 0.3))
                            : Colors.white.withValues(alpha: 0.06),
                        borderRadius: BorderRadius.circular(8),
                        border: Border.all(
                          color: isActive
                              ? (action == 'DONE' ? Colors.green : AppTheme.current.primaryColor)
                              : Colors.transparent,
                          width: 2,
                        ),
                      ),
